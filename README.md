...
CV upload → extract → structured analysis. Course demo for **CO3065 — Advanced Software Engineering** (HCMUT).

The product HTTP contracts from the old Spring app are preserved. The implementation is a pnpm/turbo monorepo shaped like a small Fleet-style TypeScript system (`apps/api` Hono layers, `apps/web` Next.js App Router, `packages/shared` Zod wire contracts).
...
Register at `/auth/register`, then `/dashboard/upload` a PDF/DOCX/DOC. Extract is accepted immediately (`Task accepted`); the processing page polls `GET /api/cv/data/{file_id}` until analysis exists.

## Layout

```
apps/api          Hono API, port 8090
  src/platform/   http, auth, db, storage, queue, llm, extract
  src/modules/    users | cv | system   (routes → handlers → service → repo)
apps/web          Next.js App Router, port 3000
packages/shared   Zod wire contracts (snake_case JSON)
```
...
| LLM | **deterministic stub** filling `basic_info`, `education`, `work_experience`, `skills`, `certificates_languages`, `analysis_result` | `GEMINI_API_KEYS` (comma-separated) |
...
CV: `POST /api/cv/upload` (multipart `file`, 201), `POST /api/cv/extract/{file_id}` (`Task accepted`), `GET /api/cv/supported-types`, `GET /api/cv/data/{file_id}`, `GET /api/cv/list`.
