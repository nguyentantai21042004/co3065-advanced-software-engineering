...
function ProcessingInner() {
  ...
  const steps = [
    {
      id: 'upload',
      title: '1. Lưu trữ tài liệu',
      desc: 'Tài liệu đã được tiếp nhận và lưu an toàn vào kho hồ sơ.',
      done: true,
      active: false,
      icon: FileText,
    },
  ...
  if (phase === 'error') {
    return (
      <Card className="mx-auto max-w-lg p-6 text-center border-red-200 bg-white shadow-soft-sm">
    ...
    <div className="mx-auto max-w-2xl space-y-5">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1.5">
          <Badge
            variant={phase === 'done' ? 'success' : 'brand'}
            size="sm"
            dot={phase !== 'done'}
          >
            {phase === 'done' ? 'Hoàn tất phân tích' : 'Đang xử lý'}
          </Badge>
          <span className="text-xs text-slate-300">·</span>
          <span className="text-xs text-slate-500 font-mono tabular-nums">Thời gian: {elapsed}s</span>
        </div>
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">
          Tiến trình đánh giá hồ sơ
        </h1>
        <p className="mt-0.5 text-xs text-slate-500 truncate">
          Mã hồ sơ: <span className="font-mono text-slate-700">{fileId}</span>
        </p>
      </div>

      {/* File Meta */}
      <Card className="p-4 bg-white shadow-soft-xs flex items-center justify-between">
      ...
      {phase === 'done' && (
        <Card className="p-5 border-emerald-200 bg-white shadow-soft">
