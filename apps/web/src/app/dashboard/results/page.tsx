...
function ResultsInner() {
  ...
  if (error || !data) {
    return (
      <PageFrame>
        <PageHeader
          breadcrumbs={[{ label: 'Lịch sử hồ sơ', href: '/dashboard/history' }, { label: 'Lỗi' }]}
          title="Không tìm thấy báo cáo"
        />
        <PageScroll mode="scroll">
          <PageContent width="form" className="py-8">
  ...
  const candidateName = String(info.name || getCurrentFile()?.name || 'Alex Rivera');
  const targetDomain = report?.domain_inference?.domain ?? 'Kỹ sư Phần mềm & Công nghệ';

  return (
    <PageFrame>
      {/* Fleet Full-Width PageHeader */}
      <PageHeader
        breadcrumbs={[
          { label: 'Lịch sử hồ sơ', href: '/dashboard/history' },
          { label: candidateName },
        ]}
        title={candidateName}
        meta={
      ...
      <PageScroll mode="scroll">
        <PageContent width="container" className="space-y-5">
            ...
            <div className="space-y-5 animate-fade-in">
              {report ? (
                <>
                  ...
                  <Card className="p-5 sm:p-6 shadow-soft-xs bg-white border border-slate-200">
                  ...
                  <Card className="p-5 sm:p-6 border border-slate-200 bg-white shadow-soft-xs">
          ...
          {activeTab === 'raw' && (
            <Card className="p-5 sm:p-6 shadow-soft-xs bg-white border border-slate-200 animate-fade-in">
